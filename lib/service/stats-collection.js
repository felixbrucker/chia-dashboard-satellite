...
class StatsCollection {
  ...
  async init() {
    ...
    await this.tryUntilSucceeded(this.ensureWalletIsLoggedIn.bind(this));
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      const isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE[service]);
